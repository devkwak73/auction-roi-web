...
export default function AdminPage() {
    ...
    const handleBulkApprove = async () => {
        ...
        if (!error) {
            alert(`${selectedUserIds.length}명의 사용자가 승인되었습니다.`);
            setSelectedUserIds([]);
            fetchUsers();
        } else {
            ...
            <div className="card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px' }}>
                        <button onClick={handleSelectAll} className="button button-outline">
                ...
                {loading ? (
                    <p style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>로딩 중...</p>
                ) : users.length === 0 ? (
                    <p style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>대기 중인 사용자가 없습니다.</p>
                ) : (
